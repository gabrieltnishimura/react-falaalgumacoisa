...
function LongPressButton(props: LongPressButtonInput) {
  ...
  return (
    <div onContextMenu={ignoreContextDropdown}>
      <div>
        <span className={contextStyles.disable}>
          {recording ? 'Solte para gravar' : 'Pressione o microfone para falar'}
        </span>
      </div>
      <div
        ref={imageRef}
      >
        <svg className={recording ? styles.recordingLongPressButtonIcon : styles.longPressButtonIcon}
